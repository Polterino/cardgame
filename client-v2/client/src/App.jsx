    ...
    return (
      <div className="min-h-screen bg-green-800 flex items-center justify-center p-4 font-mono text-white">
        <div className="bg-green-900 p-8 rounded-lg shadow-xl max-w-md w-full border border-green-700">
          <h1 className="text-4xl mb-6 text-center font-bold text-yellow-400">StrozzaTrevisana</h1>
          ...
          {gameState.players.filter(p => p.id !== socket.id).map(p => (
            <div key={p.id} className={`flex flex-col items-center p-2 rounded ${gameState.players[gameState.currentTurn].id === p.id ? 'bg-yellow-500/20 ring-2 ring-yellow-400' : 'bg-green-900/50'}`}>
              ...
              <div className="mt-2 flex -space-x-4">
                {/* Back of cards */}
                {p.hand.map((c, i) => (
                  <div key={i} className="w-8 h-12 bg-red-700 rounded border border-white shadow-sm flex items-center justify-center text-xs">
                    {/* Blind Round Logic: If cardsPerHand is 1, we show value. Else Back */}
                    {gameState.cardsPerHand === 1 ? (
                      <span className={`${SUIT_COLORS[c.suit]} bg-white w-full h-full flex items-center justify-center`}>
                        {c.value}<br/>{SUIT_ICONS[c.suit]}
                      </span>
                    ) : '🂠'}
                  </div>
                ))}
        ...
        <div className="h-48 w-full flex items-center justify-center mb-8">
          <div className="relative w-64 h-48 bg-green-700/30 rounded-full border-4 border-green-900/50 flex items-center justify-center">
            {gameState.currentRoundCards.map((play, idx) => (
              <div 
                key={idx} 
                className="absolute w-20 h-28 bg-white rounded shadow-xl flex flex-col items-center justify-center border border-gray-300 transform transition-all"
                style={{ 
                  transform: `rotate(${idx * 20 - (gameState.currentRoundCards.length * 10)}deg) translateY(${idx%2===0 ? -5 : 5}px)` 
                }}
              >
                <span className={`text-2xl ${SUIT_COLORS[play.card.suit]}`}>{SUIT_ICONS[play.card.suit]}</span>
                <span className={`font-bold ${SUIT_COLORS[play.card.suit]}`}>{play.card.value}</span>
                {play.mode === 'high' && <span className="text-[8px] text-yellow-600 font-bold uppercase">High</span>}
                {play.mode === 'low' && <span className="text-[8px] text-gray-500 font-bold uppercase">Low</span>}
                <span className="text-[8px] text-gray-400 absolute bottom-1 truncate max-w-full px-1">
                  {gameState.players.find(p=>p.id === play.playerId)?.username}
                </span>
              </div>
            ))}
          </div>
        ...
        <div className="mt-auto w-full">
          ...
          <div className="flex justify-center -space-x-4 pb-4 overflow-x-auto min-h-[140px]">
            {me.hand.map((card, idx) => {
              // Check playability (basic visual feedback only, server enforces rules)
              const isPlayable = gameState.phase === 'PLAYING' && isMyTurn;
              
              // Blind Round Logic: I see '?'
              ...
              return (
                <div 
                  ...
                  className={`
                    w-24 h-36 rounded-lg shadow-xl border-2 flex flex-col items-center justify-center transition-transform cursor-pointer relative bg-white
                    ${isPlayable && !isBlind ? 'hover:-translate-y-4 border-yellow-400 hover:z-10' : 'border-gray-300'}
                    ${isBlind ? 'bg-red-700' : ''}
                  `}
                >
                  {!isBlind ? (
                    <>
                      <span className={`text-4xl ${SUIT_COLORS[card.suit]}`}>{SUIT_ICONS[card.suit]}</span>
                      <span className={`font-bold mt-2 ${SUIT_COLORS[card.suit]}`}>{card.value}</span>
                      <span className="absolute top-1 left-1 text-xs text-gray-400">{card.suit[0]}</span>
                    </>
                  ) : (
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg text-center text-black">
            ...
            <div className="flex gap-4">
              <button onClick={() => confirmAce('high')} className="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">
                Highest Card (Win)
              </button>
              <button onClick={() => confirmAce('low')} className="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700">
                Lowest Card (Lose)
              </button>
            </div>
          </div>
      ...
      {gameState.phase === 'GAME_OVER' && (
        <div className="fixed inset-0 bg-black/90 flex flex-col items-center justify-center z-50 overflow-auto p-4">
          <div id="scoreboard" className="bg-green-900 border-4 border-yellow-500 p-8 rounded-xl max-w-lg w-full text-center">
            ...
            <div className="mt-6 text-green-400 text-sm">StrozzaTrevisana Game - {gameState.code}</div>
          </div>
